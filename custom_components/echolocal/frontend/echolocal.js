...
`;var je=26,Tt=135,Rt=270,_=100,k=100,K=84,Et=38,T=class extends v{constructor(){super(...arguments);this.level="";this.floor="";this.gate="";this.mode="";this.muted=!1;this.held=null;this.grab=t=>{let e=t.currentTarget;e.setPointerCapture(t.pointerId);let r=this.hass.states[this.gate]?.attributes??{},n=r.min??0,o=r.max??20,c=r.step??1,d=this.number(this.floor)??0,h=this.number(this.level)??0,g=Math.max(d+je,h+3),f=H=>{let W=e.getBoundingClientRect(),qi=H.clientX-W.left-W.width/2,Bi=H.clientY-W.top-W.height/2,Bt=Math.atan2(Bi,qi)*180/Math.PI-Tt;for(;Bt<0;)Bt+=360;let Ki=re(Math.min(Bt,Rt)/Rt);return Math.max(n,Math.min(o,Math.round(Ki*(g-d)/c)*c))},w=H=>{this.held=f(H)},$=H=>{e.removeEventListener("pointermove",w),e.removeEventListener("pointerup",$),e.removeEventListener("pointercancel",$);let W=f(H);this.held=null,this.hass.callService("number","set_value",{entity_id:this.gate,value:W})};e.addEventListener("pointermove",w),e.addEventListener("pointerup",$),e.addEventListener("pointercancel",$),this.held=f(t)}}render(){let t=this.number(this.level),e=this.number(this.floor),r=this.held??this.number(this.gate);if(t===null||e===null||r===null)return p;let n=this.hass.states[this.mode],o=Fe(n?.state),c=Math.max(e+je,t+3),d=re((t-e)/(c-e)),h=re(r/(c-e)),g=t>=e+r&&!this.muted;return a`
    ...
    `}number(t){let e=Number(this.hass?.states?.[t]?.state);return Number.isFinite(e)?e:null}};T.styles=b(De),l([u({attribute:!1})],T.prototype,"hass",2),l([u()],T.prototype,"level",2),l([u()],T.prototype,"floor",2),l([u()],T.prototype,"gate",2),l([u()],T.prototype,"mode",2),l([u({type:Boolean})],T.prototype,"muted",2),l([m()],T.prototype,"held",2),T=l([y("echolocal-array")],T);function Fe(s){let i=(s??"").toLowerCase();return i.includes("center")||i.includes("centre")?"one":i.includes("beam")?"beam":"sum"}function mr(s,i){return[[_,k],...Array.from({length:6},(e,r)=>{let n=(-90+r*60)*Math.PI/180;return[_+Et*Math.cos(n),k+Et*Math.sin(n)]})].map(([e,r],n)=>x`<circle class="capsule" data-on=${String(!i&&(s!=="one"||n===0))}
    ...
    ${i.map(([t,e],r)=>x`<circle class="capsule" data-on=${String(s!=="one"||r===0)}
      ...
      x2=${(_+Et*Math.cos(t)).toFixed(1)} y2=${(k+Et*Math.sin(t)).toFixed(1)}></line>`})}function vr(){return x`<path class="beam" d="M${_} ${k}C${_-34} ${k-30} ${_-24} ${k-66} ${_} ${k-66}C${_+24} ${k-66} ${_+34} ${k-30} ${_} ${k}Z"></path>`}function We(){let s=Tt*Math.PI/180,i=(Tt+Rt)*Math.PI/180;return`M${(_+K*Math.cos(s)).toFixed(2)} ${(k+K*Math.sin(s)).toFixed(2)}
