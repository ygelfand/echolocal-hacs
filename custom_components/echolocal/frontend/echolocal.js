 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */var yt=12,Ie=2.2,F=100,q=100,De=500;function Ue(r,i){let t=Array.from({length:yt},(e,s)=>{let n=-90+360/yt*s+Ie/2,o=-90+360/yt*(s+1)-Ie/2;return hs(93,82,n,o)});return x`
    <svg viewBox="0 0 200 200" role="img" aria-label="Echo Dot">
      ...
      ${se(F,q-46,x`<path d="M-4.5 0h9M0 -4.5v9"></path>`,"Volume up",()=>i.volume(1))}
      <g
        class="btn"
      ...
      ${se(F,q+46,x`<path d="M-4.5 0h9"></path>`,"Volume down",()=>i.volume(-1))}
  ...
  `}function ps(){return x``}function hs(r,i,t,e){let s=($,M)=>{let j=M*Math.PI/180;return[(F+$*Math.cos(j)).toFixed(2),(q+$*Math.sin(j)).toFixed(2)]},[n,o]=s(r,t),[l,d]=s(r,e),[h,g]=s(i,e),[f,w]=s(i,t);return`M${n} ${o}A${r} ${r} 0 0 1 ${l} ${d}L${h} ${g}A${i} ${i} 0 0 0 ${f} ${w}Z`}var je=`:host {
  display: block;
}
...
`;var Fe=26,Mt=135,Ht=270,_=100,k=100,G=84,Pt=38,H=class extends v{constructor(){super(...arguments);this.level="";this.floor="";this.gate="";this.mode="";this.muted=!1;this.held=null;this.grab=t=>{let e=t.currentTarget;e.setPointerCapture(t.pointerId);let s=this.hass.states[this.gate]?.attributes??{},n=s.min??0,o=s.max??20,l=s.step??1,d=this.number(this.floor)??0,h=this.number(this.level)??0,g=Math.max(d+Fe,h+3),f=M=>{let j=e.getBoundingClientRect(),Bi=M.clientX-j.left-j.width/2,Ki=M.clientY-j.top-j.height/2,Kt=Math.atan2(Ki,Bi)*180/Math.PI-Mt;for(;Kt<0;)Kt+=360;let Gi=re(Math.min(Kt,Ht)/Ht);return Math.max(n,Math.min(o,Math.round(Gi*(g-d)/l)*l))},w=M=>{this.held=f(M)},$=M=>{e.removeEventListener("pointermove",w),e.removeEventListener("pointerup",$),e.removeEventListener("pointercancel",$);let j=f(M);this.held=null,this.hass.callService("number","set_value",{entity_id:this.gate,value:j})};e.addEventListener("pointermove",w),e.addEventListener("pointerup",$),e.addEventListener("pointercancel",$),this.held=f(t)}}render(){let t=this.number(this.level),e=this.number(this.floor),s=this.held??this.number(this.gate);if(t===null||e===null||s===null)return p;let n=this.hass.states[this.mode],o=qe(n?.state),l=Math.max(e+Fe,t+3),d=re((t-e)/(l-e)),h=re(s/(l-e)),g=t>=e+s&&!this.muted;return a`
    ...
    ${i.map(([t,e],s)=>x`<circle class="capsule" data-on=${String(r!=="one"||s===0)}
      ...
      x2=${(_+Pt*Math.cos(t)).toFixed(1)} y2=${(k+Pt*Math.sin(t)).toFixed(1)}></line>`})}function bs(){return x`<path class="beam" d="M${_} ${k}C${_-34} ${k-30} ${_-24} ${k-66} ${_} ${k-66}C${_+24} ${k-66} ${_+34} ${k-30} ${_} ${k}Z"></path>`}function Be(){let r=Mt*Math.PI/180,i=(Mt+Ht)*Math.PI/180;return`M${(_+G*Math.cos(r)).toFixed(2)} ${(k+G*Math.sin(r)).toFixed(2)}
    A${G} ${G} 0 1 1 ${(_+G*Math.cos(i)).toFixed(2)} ${(k+G*Math.sin(i)).toFixed(2)}`}function ys(r){let i=(Mt+r*Ht)*Math.PI/180,t=G-8,e=G+8;return x`<line class="notch"
...
`;var le=135,ce=270,Ot=100,Nt=100,ct=78,Bs={White:"mdi:grain",Pink:"mdi:blur",Brown:"mdi:waveform",Rain:"mdi:weather-pouring",Ocean:"mdi:waves",Brook:"mdi:water",Wind:"mdi:weather-windy",Fire:"mdi:fireplace",Crickets:"mdi:bug-outline",Fan:"mdi:fan",Cabin:"mdi:airplane"},dt="None",Y=class extends v{constructor(){super(...arguments);this.player="";this.jack="";this.grab=t=>{let e=t.currentTarget;e.setPointerCapture(t.pointerId);let s=d=>{let h=e.getBoundingClientRect(),g=d.clientX-h.left-h.width/2,f=d.clientY-h.top-h.height/2,w=Math.atan2(f,g)*180/Math.PI-le;for(;w<0;)w+=360;let $=Math.max(0,Math.min(1,Math.min(w,ce)/ce));return Math.round($*30)/30},n=d=>this.hass.callService("media_player","volume_set",{entity_id:this.player,volume_level:s(d)}),o=d=>n(d),l=d=>{e.removeEventListener("pointermove",o),e.removeEventListener("pointerup",l),e.removeEventListener("pointercancel",l),n(d)};e.addEventListener("pointermove",o),e.addEventListener("pointerup",l),e.addEventListener("pointercancel",l),n(t)}}render(){let t=this.hass.states[this.player];if(!t)return p;let e=Number(t.attributes.volume_level??0),s=t.attributes.is_volume_muted===!0,n=this.jack?this.hass.states[this.jack]?.state==="on":!1;return a`
      ...
      <div class="side">
        <div class="state">${Ks(t.state)}</div>
        <div class="badges">
...
/* A broken entry reads as broken whether or not anything picked it. */
.word[data-used="true"] {
...
.word[data-bad="true"],
.word[data-bad="true"][data-used="true"] {
...
/* The id can be long, so it gives up its width first and says the rest on hover. */
.id {
  flex: 1 1 6ch;
  min-width: 0;
  overflow: hidden;
...
`;async function ye(r){try{return(await r.callWS({type:"echolocal/wake_words/list"}))?.wake_words??[]}catch{return[]}}var R=class extends v{constructor(){super(...arguments);this.inUse=new Set;this.words=[];this.over=!1;this.busy=!1;this.said="";this.asked=!1;this.dropped=t=>{t.preventDefault(),this.over=!1,this.add(t.dataTransfer?.files??null)}}updated(){this.asked||!this.hass||(this.asked=!0,this.refresh())}render(){return a`
    ...
    `}row(t){let e=[t.type||"no type",t.size?`${Math.round(t.size/1024)} KB`:"no model",...t.trained_languages.length?[t.trained_languages.join(", ")]:[]],s=this.inUse.has(t.wake_word);return a`<div
      class="word"
      data-bad=${String(t.problems.length>0)}
      data-used=${String(s)}
    >
      <div class="said">
        <ha-input
      ...
      <div class="about">
        <div class="id" title=${t.id}>${t.id}</div>
        <div class="files">
          ${t.model_url?a`<a class="pill" href=${t.model_url} download title=${`${t.id}.tflite`}>
                <ha-icon icon="mdi:tray-arrow-down"></ha-icon>tflite
              </a>`:p}
          ${t.config_url?a`<a class="pill" href=${t.config_url} download title=${`${t.id}.json`}>
                <ha-icon icon="mdi:tray-arrow-down"></ha-icon>json
              </a>`:p}
        </div>
        <div class="facts">${e.join(" \xB7 ")}</div>
      </div>
      <div class="buttons">
